...
@Repository
public interface ArticleCsvRepository extends PagingAndSortingRepository<ArticleCsv, Long> {

   long deleteByCsvId(Long csvId);

   Page<ArticleCsv> findAllByCsvIdIs(Long loteId, Pageable page);

   long deleteByLoadDateBefore(LocalDate loadDate);
   ...
   @Query(value = "SELECT \n" +
           ...
           "            UAA.estado_carga as estadoCarga\n" +
           "    FROM pricing.UE_ARTICULO_AMPLIADO_CARGA UAA INNER JOIN pricing.ue_estados ue ON UAA.id_estado = ue.id\n" +
           "where (:csvId =-1 or UAA.csv_id = :csvId) and \n" +
           ...
           "and (:idEstado =-1 or UAA.id_estado = :idEstado)",
           countQuery = "SELECT COUNT(UAA.ID) " +
                   "FROM pricing.UE_ARTICULO_AMPLIADO_CARGA UAA INNER JOIN pricing.ue_estados ue ON UAA.id_estado = ue.id " +
                   "where (:csvId =-1 or UAA.csv_id = :csvId)\n" +
                   ...
                   "and(:fechaMax is null or TRUNC(UAA.fecha_inicio) <= :fechaMax)\n"+
                   "and(:fechaMin is null or TRUNC(UAA.fecha_inicio) >= :fechaMin)"+
                   "and (:idEstado =-1 or UAA.id_estado = :idEstado)",nativeQuery = true)
   Page<ArticleExtendLoadDto> visualizarRegistrosVigentes(@Param("csvId") Long csvId,
